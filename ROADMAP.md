...
**Progress (2026-06-11, stage 2)**: CSV-driven generalization done — `hardware/form_parser.py` parses any `symbolic_form` into a netlist; `python -m hardware.run_csv` evaluates and emits RTL for every unique valid form in the canonical CSVs (report: `results/hardware_csv_forms_report.md`). Finding: all 152 valid rows collapse to the two canonical forms, so the emitted RTL covers 100% of released valid solutions; parsed netlists match tree-extracted numerics bit-for-bit. **Progress (2026-06-11, stage 3)**: OSS CAD Suite installed (`~\tools\oss-cad-suite`); both blocked items done. Icarus bit-equality sweep (`python -m hardware.sim_check`): RTL matches the Python model bit-for-bit, 256/256 points per design. yosys+nextpnr on UP5K: exp_d2 586 LCs (11%), ln_d4 3193 LCs (60%), both fit; 0 EBR inferred (combinational ROM reads map to fabric — the pipelined variant with registered reads is the path to EBR + Fmax numbers). **Progress (2026-06-11, stage 4)**: Pipelined variant done (`hardware/verilog_gen_pipelined.py`): registered EBR reads, split-bank LUTs, 3-stage gates, delay-matched chaining; bit-exact vs the Python model (sim_check, 4/4 designs). Measured: exp_d2_pipe 255 LCs / 3 EBR / 29.9 MHz (UP5K), ln_d4_pipe 1932 LCs / 13 EBR / 39.1 MHz (HX8K ct256; UP5K-fits logically but bare ports exceed sg48 pins). ~30 Msamples/s streaming. **Progress (2026-06-11, stage 5)**: Byte-serial streaming wrapper (`hardware/verilog_gen_stream.py`, 19 pins) resolved the pin blocker: both designs route on UP5K sg48 with valid `icepack` bitstreams. ln_d4_pipe_stream: 1964 LCs (37%) / 13 EBR (43%) / 16.7 MHz ≈ 5.6 Msamples/s; exp_d2_pipe_stream: 316 LCs / 3 EBR / 28.3 MHz ≈ 14 Msamples/s. sim_check: 6/6 designs bit-exact vs the Python model. The repo name's "ice40" promise is now realized end-to-end in silicon-ready form. Remaining: board demo (pcf + iceprog + host script) when hardware is on hand.
