...
**Key results visible in the data (v2.3 diagnostic snapshot, post 20-seed control — PRE-FIX EMBEDDING)**:
This snapshot + the numbers derived from it are the pre-fix diagnostic corpus (the ln warm/curriculum collapse to trivial was caused by the init/grow wiring bug fixed in the subsequent patches). For the corrected post-fix behaviour and data (full 20-seed with fixed code + pretrain_form), see results/v2.4_postfix_snapshot/ and run the exact command in the main note's Reproducibility section.
