...
For ln at d=5 (the over-representational-depth cell that drops from ~90% at representational depth 4), the same family of interventions yields 0% valid recovery for warm and curriculum across the final data (66 blind + 66 warm-spine + 54 curriculum+reanneal). Every curriculum run (54/54) and every warm-spine run collapses to the identical trivial form `eml(1,eml(1,1))`. A dedicated 10-seed / 400-epoch reanneal tuning run (completed) and the ln portion of the 20-seed control (completed) also produced 0/10 and 0/20 valid for curriculum, with the same 100% collapse. This is a first-class boundary result: the spine-embedding curriculum successfully injects the known structure (per the `grow_from_shallow` implementation), yet the subsequent optimization still selects a strong competing constant attractor. Longer reanneal (400 epochs) and unbalanced-tree support are the immediate next levers. A "v2.3 basin" data snapshot and reproducibility scripts accompany this note.
