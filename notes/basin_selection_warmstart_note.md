...
<p class="hebrew-date" dir="rtl" lang="he">

<!-- TODO: insert vocalized Hebrew date: day-letter geresh, vocalized month, year gershayim --></p>
...
v1.2 . DOI: *to be minted on Zenodo deposit*

This work is licensed under CC BY 4.0 (https://creativecommons.org/licenses/by/4.0/). Code released under the MIT license.

Technical note accompanying *Valid and False Snapping in EML Expression Trees: The Basin Selection Problem* (the "companion paper"; v2.3 at time of writing) and its basin-selection warm-start note (v1.0, DOI 10.5281/zenodo.20671272). This note reports the hardware study of the repository `eml-ice40-cybenko`. Companion artifacts: `mlp/` (the MLP and ROM baselines), `hardware/` (the snapped-EML units), `results/mlp_pareto.csv` (synthesis sweep), `notes/figure_symbolic_vs_mlp.png`.

------

## Abstract

Deploying a *learned* scalar function on a small FPGA forces a choice between two unattractive corners. A direct lookup ROM, `ROM[x] = quantize(f(x))`, is bit-exact to the true function's fixed-point image and exhaustively verifiable, but its size is set by the input wordlength and grows as the product of per-input domains. A quantized MLP is compact in parameters, but on fabric where every multiply maps into LUTs and carry chains, and it is exact only to its own trained approximation floor. We show a third option in the realizable gap between them: an elementwise-ML (EML) expression tree recovered by symbolic regression and then argmax-*snapped* to a fixed-function datapath, where each gate is the literal `exp(a) − ln(b)` with inputs routed from `{1, x, child}`.

On a Lattice iCE40 in fabric-only mode (no DSP blocks used; see §3), the two canonical recovered primitives, exp `eml(x,1)` and ln `eml(1,eml(eml(1,x),1))`, are at once smaller in logic cells and more accurate than every quantized MLP we train (H ∈ {4,8,16,32}, depth ∈ {1,2}, best of 8 seeds), and bit-exact to the fixed-point image, confirmed 256/256 on a physical iCEstick. Against a direct ROM the picture sharpens: for exp the ROM *ties* the symbolic unit, but for ln it collapses to ~512 block RAMs, infeasible on any iCE40, while the symbolic ln unit fits an HX8K via leading-one range reduction. The unifying claim is that **the snapped symbolic primitive is a compressed ROM**, smallest-advantage for cheap single-input cases and growing with input wordlength and arity. We report the area-accuracy Pareto frontier and on-silicon confirmation through one end-to-end pipeline.

------

## 1. Introduction

A recurring problem in edge inference is realizing a learned scalar nonlinearity, an activation, a calibration curve, a recovered physical law, as compact, trustworthy hardware on a small FPGA. Two answers are obvious. Tabulate it: store `quantize(f(x))` in a ROM addressed by the input code. Or learn it: train a small quantized MLP and synthesize the MACs. Each has a well-known failure. The ROM is exact and trivially verifiable but its size is set by the input wordlength, not the function's complexity, so it explodes with precision and with the number of inputs. The MLP is parameter-compact but, when multiplies map to fabric LUTs and carry chains (the DSP-less regime), every multiply costs logic area, and, more fundamentally, it is correct only to a training-dependent approximation floor, which is a weak guarantee for certification-sensitive use.
...
- **DSP-block note.** The UP5K contains 8 SB_MAC16 hard multiply-accumulate blocks; the HX1K and HX8K do not. None of our designs use them: the symbolic unit contains no multiplies (only LUTs, adders, and EBR), and the MLP emitter writes weights as RTL constants that yosys folds into LUT logic rather than inferable multiply patterns. All LC counts in the tables below reflect fabric-only mapping (0 SB_MAC16 in every nextpnr run). The "DSP-less" framing in this note refers to this fabric-only regime, not a blanket claim that every iCE40 variant lacks DSPs.
...
Logic cells and routed Fmax are from yosys `synth_ice40` → nextpnr-ice40 on the byte-serial stream wrapper, targeting iCE40 UP5K sg48 (5280 LCs). `fp max-err` is the fixed-point error vs the true function (best of 8 seeds); `float ± std` is the float-grid max-error across the 8 seeds. Device references: HX1K 1280, UP5K 5280, HX8K 7680 LCs (logic cells); EBR is Embedded Block RAM (4 Kbit each, configurable 256x16). **LCs** is the nextpnr `ICESTORM_LC` count, printed even when placement overflows the device. A `†` marks cells whose synthesis did not finish in the 1-hour cap; for these we report a lower bound (the largest measured same-function cell, strictly larger, and losing on accuracy regardless). Every row is bit-exact 256/256 in simulation. The 316-LC exp figure here is the byte-serial stream core on UP5K; the 530-LC figure in Sec. 4.5 is the same datapath inside the iCEstick/HX8K board top (UART bridge and FIFO), measured on HX1K and HX8K.

**exp on [−2, 2] (Q8.8):**

| unit                       | LCs      | fits | Fmax (MHz) | fp max-err | float ± std (N=8) |
| -------------------------- | -------- | ---- | ---------- | ---------- | ----------------- |
| **snapped EML `eml(x,1)`** | **316**  | HX1K | 28.3       | **0.0150** | n/a               |
...
| MLP h32 d1                 | 8673     | none | n/a        | 0.0930     | 0.108 ± 0.039     |
| MLP h4 d2                  | 3587     | UP5K | 15.4       | 0.2298     | 0.739 ± 0.722     |
| MLP h8 d2                  | 11156    | none | n/a        | 0.0831     | 0.124 ± 0.045     |
| MLP h16 d2                 | 32276    | none | n/a        | 0.0930     | 0.066 ± 0.031     |
| MLP h32 d2                 | ≥32276 † | none | n/a        | 0.0480     | 0.033 ± 0.011     |

**ln on [0.1, 10] (Q10.12):**

| unit                                     | LCs      | fits | Fmax (MHz) | fp max-err  | float ± std (N=8) |
| ---------------------------------------- | -------- | ---- | ---------- | ----------- | ----------------- |
| **snapped EML `eml(1,eml(eml(1,x),1))`** | **1964** | HX8K | 16.7       | **0.00107** | n/a               |
| MLP h4 d1                                | 3066     | UP5K | 12.5       | 0.4549      | 1.338 ± 0.726     |
| MLP h8 d1                                | 4933     | UP5K | 13.4       | 0.2105      | 0.614 ± 0.307     |
| MLP h16 d1                               | 9454     | none | n/a        | 0.1461      | 0.292 ± 0.107     |
| MLP h32 d1                               | 18468    | none | n/a        | 0.0506      | 0.085 ± 0.023     |
| MLP h4 d2                                | 7281     | HX8K | n/a        | 0.2840      | 0.634 ± 0.258     |
| MLP h8 d2                                | 23319    | none | n/a        | 0.1510      | 0.540 ± 0.657     |
| MLP h16 d2                               | ≥23319 † | none | n/a        | 0.0658      | 0.243 ± 0.261     |
| MLP h32 d2                               | ≥23319 † | none | n/a        | 0.0265      | 0.059 ± 0.029     |

The headline figure (`notes/figure_symbolic_vs_mlp.png`, **Figure 1**: logic cells (x) vs fixed-point max-error (y), both functions, lower-left dominates) plots logic cells vs fixed-point max-error per function. The two snapped-EML units dominate the plane, lower area **and** lower error than the entire MLP family. The cleanest statement is on the accuracy axis alone: no MLP cell, at any width, depth, or seed in the sweep, reaches the symbolic unit's fixed-point accuracy (best MLP exp 0.048 vs 0.015; best MLP ln 0.026 vs 0.001). The mechanism is concrete: in fabric-only mode every signed multiply is built from LUTs and carry chains, and area scales with the MAC count (≈ H for depth 1, ≈ H² for depth 2). MLPs blow past the device before reaching the symbolic accuracy. The snapped-EML unit's cost is just two small interpolated LUTs per gate.
...
| ln [0.1,10] Q10.12 | **direct ROM** (65536×22) | n/a  | **512** | 0.00107    | **none**       |
...
For **ln**, the ROM **collapses**: the 22-bit Q10.12 input over [0.1, 10] needs ≈65 536 entries × 22 b. Packed by bit-count that is ~1.44 Mbit (~352 EBR), but iCE40 EBR tile as 256x16, so the table needs 65536/256 = 256 tiles deep × ceil(22/16) = 2 tiles wide = 512 EBR, 16× the 32 EBR on an HX8K, infeasible on any iCE40. The snapped-EML ln unit fits an HX8K precisely because leading-one range reduction is the *compression* of that table. The general statement: **the snapped-EML / interpolated-LUT unit is a compressed ROM**, exp shrinks a 2048-entry table to a 257-entry interpolated LUT (~16×); ln shrinks an unrealizable 512-EBR table to 13 EBR. For one narrow-input function the compression can be unnecessary (exp); its value appears exactly where the direct table fails, wide input wordlength (ln) and, decisively, **multiple inputs**, where a direct ROM grows as the *product* of per-input domains while the symbolic datapath does not. The ROM-collapse argument therefore bounds the claim rather than refuting it: symbolic ≈ ROM for cheap single-input cases, symbolic ≫ ROM as soon as input width or arity grows.
...
| ----------------------------- | ------------------------------- | ----- | ---------- | ---------------- |
| **symbolic `exp(x) − ln(y)`** | **514** (2×257, flat in `W_in`) | n/a   | **0.0248** | yes              |
...
We attempted the same on-silicon confirmation for the MLP baseline. The machinery is in place, `mlp/board.py` emits an `icestick_mlp_top` bridging the same UART to a chosen exp MLP core, but the baseline does not fit the iCEstick at all. Even the smallest, least accurate exp MLP (1→4→1, 0.40 max-error) places at 1423 ICESTORM_LCs, 111% of the HX1K. Every more-accurate MLP is larger still. On this board the comparison is therefore the sharper "symbolic is the *only* one of the two that is physically realizable." A physical MLP result requires a larger device; we add an iCE40-HX8K Breakout target (7680 LCs, `hardware/hx8k_breakout.pcf`), where the symbolic exp unit places at 530/7680 LC (7%, 69.7 MHz) and the symbolic ln unit fits too (1964 LC / 13 EBR). Only the smaller, lower-accuracy MLPs fit the HX8K; every MLP that approaches the symbolic accuracy overflows even this device. The HX8K, like the HX1K, has no hard multipliers, so MLP MACs still map to fabric LUTs and the area-dominance result is unchanged, the larger device only provides the room to confirm it.

------

## 5. Verifiability

All three units are bit-exact to their own fixed-point golden models (256/256 in simulation). The distinction is the *source* of the approximation error. The symbolic unit and the direct ROM have deterministic, analytically bounded error: for the symbolic unit, the bound is set by the interpolation grid spacing and the fixed-point format, and it is the same for every build. There is no training-noise term, no seed dependence, no float-to-fixed conversion uncertainty. The MLP's golden model is also bit-exact to its RTL, but "model == intended function" holds only to the trained approximation error, which depends on the seed, the optimizer trajectory, and the float-to-fixed cast. For certification, a deterministic bound is qualitatively stronger than a training-contingent one. This places the symbolic unit and the ROM in one error class and the MLP in another; among the deterministically bounded units, the symbolic one is the realizable one as soon as input width or arity grows (§4.3, §4.4).
...
The idea: the snapped symbolic primitive computes the same input-output mapping as the direct ROM but stores fewer entries (257 vs 2048 for exp, 13 EBR vs 512 for ln). A direct ROM is the trivially exact unit; range reduction and interpolated LUTs are the standard compressions of it; symbolic regression *recovers* a form that, after snapping, is exactly such a compressed unit, without the engineer specifying the reduction. The three corners, ROM, MLP, symbolic, are then not three unrelated baselines but a single axis: exactness with no compression (ROM), compression with no exactness (MLP), and exact compression (symbolic).

The consequence: in the fabric-only regime the symbolic unit is the only option that is compact, bit-exact, *and* realizable for both functions. The ROM is area-superior for exp (§4.3) but unrealizable for ln; the MLP is unrealizable at competitive accuracy on either. For edge inference where the nonlinearity must be both small and certifiable, the workflow "discover the symbolic form via gradient-based symbolic regression, snap it, compile it" is a practical design route for FPGA engineers targeting DSP-less or DSP-constrained fabric, and the deterministic-error-bound argument (§5) survives even on a DSP-rich part where the area gap would shrink.
...
- **Fabric-only regime.** The HX1K and HX8K have no hard multipliers; the UP5K has 8 SB_MAC16 DSP blocks, unused here (§3). On a DSP-rich FPGA the MLP area gap would shrink (the verifiability and exactness arguments are unchanged).
- **Standard hardware approximations not benchmarked.** CORDIC, minimax polynomials, and piecewise-linear interpolation are well-studied alternatives for exp and ln on FPGAs. This note does not compare against them. The contribution is that the symbolic form was *automatically recovered* by gradient-based symbolic regression, not hand-derived. The ROM and MLP baselines bracket the design space (exact tabulation vs learned approximation) to show where the discovered form lands; they do not claim to cover the full zoo of hand-engineered function units.
- **MLP baseline is intentionally simple.** ReLU activations, uniform width/depth, no architectural search or hardware-aware training. It establishes the learned-approximation corner of the design space, not the frontier of neural function approximation on FPGAs.
...
A snapped EML expression tree, recovered by symbolic regression, compiles to a tiny fixed-point function unit. On iCE40 in fabric-only mode it dominates a quantized-MLP baseline on area and accuracy at once, is bit-exact to the true function (confirmed 256/256 on a physical iCEstick), and against a direct ROM is area-superior for ln (where the ROM needs 512 block RAMs and fits no iCE40) while conceding area on the cheap single-input exp case (§4.3). Its advantage grows with input wordlength and arity (measured at k=2, §4.4; extrapolated beyond on `O(sum)` vs `O(product)` scaling). Symbolic-regression discovery is a practical route to compact, deterministically bounded function units for certification-sensitive edge inference. All claims are reproducible from the scripts in `mlp/` and `hardware/`.
