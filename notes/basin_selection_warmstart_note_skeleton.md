...
# 20-seed control (final precision; in progress)
...
| exp      | 3     | curriculum (grow_from_shallow + reanneal_extra_capacity) | 28 (cumul) | **57% cumul** (100% in 12-seed reanneal batch) | In the dedicated 12-seed/1200-epoch batch with reanneal: 12/12 (100%) clean `eml(x,1)`. The re-anneal pass on extra selectors (spine frozen) after embedding solves the "extra levels did not fully collapse" issue observed in non-reanneal curriculum runs. |
...
| ln       | 5     | curriculum (grow_from_shallow + reanneal_extra_capacity) | 34 (cumul) | 0% | *Every* run (34/34) collapses to the identical trivial form `eml(1,eml(1,1))`. The spine embedding is performed (see `grow_from_shallow` for the 'f'-chain construction + reanneal spine freeze), yet the optimizer still selects the strong constant attractor. A dedicated 10-seed / 400-epoch longer-reanneal tuning run (completed) also gave 0/10 valid, all `eml(1,eml(1,1))`. Full 20-seed control (incl. ln curriculum) still in progress. |
...
| curriculum (grow_from_shallow + reanneal_extra_capacity) | 34 | 0 | `eml(1,eml(1,1))` | 100% | Includes the completed 10-seed/400-epoch reanneal tuning batch (all 10 also `eml(1,eml(1,1))`). The curriculum path (`basin_warmstart.py:152-165`) pre-trains at depth 4, calls `grow_from_shallow` (eml_layer_v2.py:370: copies shallow first gates to spine + extends 'f' chain + forces siblings to '1'), then `reanneal_extra_capacity` (freezes `lev[0]` spine with extreme bias, L1-sharpens only extras on dummy zero target). The embedded spine is present at the start of main training, yet every final snapped tree (34/34) is the identical trivial constant. Even 2× reanneal epochs produced no change. |
...
- For ln at d=5 the picture is qualitatively different and is treated as a first-class boundary result. Across current rows (55 blind, 46 warm-spine, 34 curriculum+reanneal), **valid recovery is 0% in every mode** (a few lucky blinds have appeared in the overall blind cumulative, but 0 in the dedicated batches). Moreover, *every* curriculum run (34/34 = 100%) and every warm-spine run (46/46 = 100%) collapses to the *identical* trivial form `eml(1,eml(1,1))`. See the dedicated forms table below. The `grow_from_shallow` implementation for ln (basin_warmstart.py:152 shallow at depth 4 + tree.grow_from_shallow at 159, then reanneal at 163) does construct the expected 'f'-spine (copying first-gate symbols from the shallow levels onto `self.levels[si][0]`, extending the 'f' chain upward, and forcing siblings to '1' — eml_layer_v2.py:370-393) and the reanneal pass freezes that spine (first gate per level) while sharpening extras. A dedicated 10-seed/400-epoch longer-reanneal tuning run (just completed) also produced 0/10 valid for curriculum (and 0/10 warm), with the exact same 100% collapse to `eml(1,eml(1,1))`. Yet the subsequent full `train_eml` still drives the tree to the same strong constant attractor. This is not noisy failure — it is a perfectly consistent selection of a competing basin even after doubling the targeted reanneal on extra capacity. The result shows that the warm-start + curriculum + reanneal recipe that is highly effective for exp is not yet sufficient for ln at over-representational depth under the current balanced-tree protocol. The full 20-seed control (including curriculum for ln:5) is still in progress to increase precision on this boundary case.
